...
for row in rows:
    ...
    if config['fetch_files'] is True:
        obj_url = config['islandora_base_url'] + '/islandora/object/' + pid + '/datastream/OBJ/download'
        row_count += 1
...
# Write the CSV file.
fields = csv_header_row.split(',')
if config['id_field'] not in fields:
    csv_header_row = f"{config['id_field']},{csv_header_row}"
    for index, value in enumerate(csv_output):
        csv_output[index] = f"{index + config['id_start_number']},{value}"

with open(config['csv_output_path'], 'w', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',',
                        quotechar='|', quoting=csv.QUOTE_MINIMAL)
    writer.writerow(csv_header_row.split(','))
