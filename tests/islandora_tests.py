...
class TestExportCSVWithAdditionalFiles(unittest.TestCase):
    def setUp(self):
        ...
        self.current_dir = os.path.dirname(os.path.abspath(__file__))
        self.test_assets_dir = os.path.join(self.current_dir, 'assets', 'additional_files_test')
        ...
        with open(self.rollback_csv, 'r') as f:
            self.nids = [line.strip() for line in f.readlines()[3:]]  # Skip header and comments
            ...
            for row in reader:
                ...
                for url_field in ['file', 'preservation', 'transcript']:
                    if row[url_field]:  # Handle empty fields
                        ...
                        self.assertEqual(response.status_code, 200, f"Failed to access {url_field} URL: {row[url_field]}")
            ...
            for row in reader:
                ...
                for field in ['file', 'preservation', 'transcript']:
                    if row[field]:  # Handle empty fields
                        ...
                        self.assertTrue(os.path.exists(file_path), f"File {row[field]} not found in export directory")
