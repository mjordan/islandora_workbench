...
class TestSplitGeolocationString(unittest.TestCase):
    ...
    def test_split_geolocation_string_multiple(self):
        config = {'subdelimiter': '|'}
        res = workbench_utils.split_geolocation_string(config, '30.16667, -120.93333|50.1,-120.5')
    ...
    def test_split_geolocation_string_multiple_at_sign(self):
        config = {'subdelimiter': '@'}
        res = workbench_utils.split_geolocation_string(config, '49.16667, -123.93333@50.1,-120.5')
        self.assertDictEqual(res[0], {'lat': '49.16667', 'lng': '-123.93333'})
        self.assertDictEqual(res[1], {'lat': '50.1', 'lng': '-120.5'})
...
class TestSetMediaType(unittest.TestCase):
    ...
    def test_multi_types_set_media_type(self):
        res = workbench_utils.set_media_type('/tmp/foo.txt', self.multi_types_config_yaml)
        self.assertEqual(res, 'extracted_text')

        res = workbench_utils.set_media_type('/tmp/foo.tif', self.multi_types_config_yaml)
        self.assertEqual(res, 'file')

        res = workbench_utils.set_media_type('/tmp/foo.mp4', self.multi_types_config_yaml)
        self.assertEqual(res, 'video')

        res = workbench_utils.set_media_type('/tmp/foo.png', self.multi_types_config_yaml)
        self.assertEqual(res, 'image')

        res = workbench_utils.set_media_type('/tmp/foo.pptx', self.multi_types_config_yaml)
        self.assertEqual(res, 'document')

        res = workbench_utils.set_media_type('/tmp/foo.xxx', self.multi_types_config_yaml)
        self.assertEqual(res, 'file')

    def test_single_type_set_media_type(self):
        res = workbench_utils.set_media_type('/tmp/foo.bar', self.single_type_config_yaml)
