...
def _cleanup_row(row_to_clean: dict):
...
def _get_rows(query_url):
    """Get rows from Solr."""
    """Parameters:
    query_url: str: The Solr query URL.
    -------
    Returns:
    list: The rows from Solr.
    """
    try:
...
def _process_row(row: dict, row_id: int, failed_list: list) -> dict:
...
def _process_block(
    ...
    rows = _get_rows(solr_request_string)
    for row in rows:
        inner_row_count += 1
        pbar(inner_row_count)
        try:
            row = _process_row(row, inner_row_count, failed_list)
            writer.writerow(row)
...
row_count = 1
total_processed = numFound if config["paginate"] else utils.get_config()["rows"]
...
with open(config["csv_output_path"], "w", newline="") as csvfile:
    ...
    if config["paginate"]:
        ...
        while row_count <= numFound:
            max_row = row_count + utils.get_config()["rows"] - 1
            if max_row > numFound:
                max_row = numFound
            utils.get_logger().info(
                f"Processing rows {row_count} to {max_row} of {numFound}"
            )
            row_count = _process_block(
        ...
        row_count = _process_block(metadata_solr_request, row_count, failed_pids)
    if failed_pids:
